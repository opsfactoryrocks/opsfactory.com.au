...
- Fork the repository above and clone your fork
- `molecule init --provider docker`

You'll have a a new file in the repository's root called `molecule.yml`, let's edit it and dump in some conent:
...
- We're setting the default (and preferred) verifier to Testinfra

Next we put in place some Testinfra tests. Edit the `tests/test_default.py` file and dump the following into it:
...
Now we can actually run out tests. It's true. Here is some example output based on the above `molecule.yml` and `test_default.py` files:
...
What a lovely sight. For some, this may have taken several minutes (it did for me on my connection), but for those on fibre or fast connections, it would have been much quicker. When plugged into TravisCI or another managed service, you'll probably get very good speeds when it comes to packages installations.
